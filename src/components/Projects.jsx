            ...
            {activeModalProject && (
              <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 overflow-hidden">
                ...
                <motion.div 
                  ...
                  onClick={(e) => e.stopPropagation()}
                  className="relative z-10 w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl sm:rounded-4xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 sm:p-8 shadow-2xl"
                >
                  
                  <button
                    onClick={() => setActiveModalProject(null)}
                    className="absolute top-6 right-6 p-2 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-100 transition-colors"
                  >
                    <X size={20} />
                  </button>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {activeModalProject.tags.map((tTag, idx) => (
                          <span
                            key={idx}
                            className="px-3 py-1 rounded-xl text-xs font-semibold bg-pastel-lavender-light dark:bg-slate-800 text-indigo-700 dark:text-indigo-300 border border-pastel-lavender dark:border-slate-700"
                          >
                            {tTag}
                          </span>
                        ))}
                  ...
                  <div className="mt-8 pt-6 border-t border-slate-100 dark:border-slate-800 flex flex-wrap items-center justify-between gap-3">
                    ...
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      onClick={() => setActiveModalProject(null)}
                      className="px-6 py-2.5 rounded-2xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 text-xs font-bold transition-colors border border-slate-200 dark:border-slate-700"
                    >
                      {t.modal.close}
                    </motion.button>
                  </div>
